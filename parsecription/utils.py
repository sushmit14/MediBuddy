...
class PatientInfo(BaseModel):
    """Call this with a Line that contains Weight, BP, Heart Beat Rate BPM, O2 Saturation Levels or any Medical Metric etc"""
...
class TestResult(BaseModel):
    """Call this with a Line that contains Pathological Data liek RBC, Haemoglobin, AST"""
    name: Optional[str] = Field(
        description="Name of Metric/Test/Pathological Marker")
    value: Optional[float] = Field(
        description="Value of the Pathological Marker like RBC")
    unit: Optional[str] = Field(description="Unit like g/dL")
    # range: Optional[str] = Field(
    #     description="Normal Range Mentioned")
    normal_range_lower_limit: Optional[float] = Field(
        description="Lower Limit of Normal Range")
    normal_range_upper_limit: Optional[float] = Field(
        description="Upper Limit of Normal Range")
    flag: Optional[str] = Field(
        description="If Value is within the range, set flag as Normal, else set flag as Abnormal"
    )


class MedicalConditions(BaseModel):
    """Medical Conditons or Terms like Fatty Liver, Calculus etc."""
    medical_conditions: Optional[str] = Field(
        description="Medical Terms or Conditions mentioned.")
...
class ParserModel():
    def __init__(self):
        ...
        self.functions = [
            ...
            convert_pydantic_to_openai_function(MedicalConditions),
            convert_pydantic_to_openai_function(TestResult)
        ]
